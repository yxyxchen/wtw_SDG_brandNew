...
for (sIdx in 1 : n) {
  ...
  for (bkIdx in 1: nBlock){
    ...
    if(isTrun){
      excluedTrials = which(thisTrialData$trialStartTime > (blockSecs - min(tMaxs)))
...
blockData = data.frame(id = rep(allIDs, each = nBlock), blockNum = rep( t(1 : nBlock), n),
                       ...
                       muQuitTime = muQuitTime, nQuit = nQuit, nTrial = nTrial, stdWd = stdWd, cvWd = cvWd, nExclude = nExclude)
...
>>>>>>> 0fe25d54a9de2600c2944691b72d5ee13c815da8
# plot AUC in two conditions
library("ggpubr")
blockData %>% filter(blockNum == 1) %>% ggplot(aes(condition, AUC)) + geom_boxplot() +
...
plotData %>% ggplot(aes(time, mean, color = condition,  fill = condition)) +
  ...
  myTheme + ylim(c(0, min(tMaxs) + 2))
ggsave("figures/expDataAnalysis/zTruc_wtw_timecourse.png", width = 6, height = 3)

# plot survival curve
select = (blockData$blockNum == 1)
data.frame(kmsc = unlist(kmOnGrid_[select]), time = rep(kmGrid, sum(select)),
           condition = factor(rep(blockData$condition[select], each = length(kmGrid))), levels = conditions) %>%
  group_by(condition, time) %>%
...
select = (blockData$blockNum == 1)
data.frame(value = unlist(timeReRate_[select]), time = rep(tGrid, sum(select)),
=======
