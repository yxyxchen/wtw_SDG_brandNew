...
################ monte ######################
utility = function(utiCurve, c){
  if(utiCurve == 0){
    return(c)
  }else{
    return((1 - exp(-utiCurve * c)) / utiCurve)
  }
}
risk = function(paras, cond, scheduledWait, wtwEarly){
  # parse para
  phi = paras[1]
  tau = paras[2]
  gamma = paras[3]
  ...
  for(tIdx in 1 : nTrial) {
    ...
    if(tIdx < nTrial){
      # update action values for each timestep t
      returns = sapply(1 : (T-1), function(t) gamma^(T-t-1) * nextReward + gamma^(T-t) * Viti)
      utilities = utility(utiCurve,returns)
      # returns = sapply(1 : (T-1), function(t) gamma^(T-t-1) *nextReward + gamma^(T-t) * Viti)
      # when the agent always wait and get the reward, update Qwait[1:(T-1)]
      # otherwise, update Qquit and Qwait[1 : (T-2)]      
      if(getReward){
        Gs[1 : (T-1), tIdx] = utilities[1 : (T-1)]
        deltas[1 : (T-1), tIdx] = utilities[1 : (T-1)] - Qwait[1 : (T-1)]
        Qwait[1 : (T-1)] = Qwait[1 : (T-1)] + phi*(utilities[1 : (T-1)] - Qwait[1 : (T-1)])
      }else{
        Qquit = Qquit + phi*(utilities[T-1] - Qquit)
        if(T > 2){
          Gs[1 : (T-2), tIdx] = utilities[1 : (T-2)]
          deltas[1 : (T-2), tIdx] = utilities[1 : (T-2)] - Qwait[1 : (T-2)]
          Qwait[1 : (T-2)] = Qwait[1 : (T-2)] + phi*(utilities[1 : (T-2)] - Qwait[1 : (T-2)])
        }
      }
      
      # update Viti
      junk = gamma^(iti / stepDuration) * returns[1]
      Viti = Viti + phi*( utility(utiCurve,junk) - Viti)
      
      # update Qquit by counterfactual learning
      junk = gamma^(iti / stepDuration + 1) * returns[1]
      Qquit = Qquit + phi*(utility(utiCurve,junk)- Qquit)
