data {
  ...
  vector[4] expParaMedian;
}
transformed data {
  // constant
  real phi = expParaMedian[1];
  real tau = expParaMedian[2];
  real gamma = expParaMedian[3];
  real QwaitIni = expParaMedian[4];
...
model {
  ...
  for(tIdx in 1 : N){
    ...
    if(trialEarnings[tIdx] == 0 && i == nTimePoints[tIdx]){
      ...
      values[1] = Qwaits[i, N] * Tau;
      values[2] = Qquits[N] * Tau;
